...
fn delta(base: &Snapshot, current: &Snapshot) -> anyhow::Result<Delta> {
    ...
    for (key, value) in &current.data {
        let shared = current.data.get_raw_shared(key).expect("present");
        let unchanged = base
            .data
            .get_raw_shared(key)
            .is_some_and(|previous| Arc::ptr_eq(previous, shared) || previous.as_ref() == value);
        if !unchanged {
            data.insert_shared(
                key.clone(),
                shared.clone(),
    ...
    for key in base.data.keys() {
        if current.data.get_raw_shared(key).is_none() {
