...
#[test]
fn wire_budget_includes_maximum_append_envelope_and_snapshot_base64_expansion() {
    ...
    // Base64 carries three segment bytes in four.
    let mut chunk = pairs.clone();
    chunk[2].1 = (max_transaction / 4 * 3).to_string();
    assert!(settings(&chunk).is_ok());
    chunk[2].1 = (max_transaction / 4 * 3 + 1).to_string();
