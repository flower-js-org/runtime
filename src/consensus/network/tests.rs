...
#[test]
fn snapshot_segments_carry_their_bytes_as_base64() {
    ...
    let text = serde_json::to_string(&SnapshotSegment::from(request.clone())).unwrap();
    let value: serde_json::Value = serde_json::from_str(&text).unwrap();
    assert!(value["data"].is_string(), "{text}");
    let plain = serde_json::to_string(&request).unwrap();
    assert!(text.len() * 3 < plain.len() * 2, "{} against {} bytes", text.len(), plain.len());
    let decoded: InstallSnapshotRequest<TypeConfig> =
        serde_json::from_str::<SnapshotSegment>(&text).unwrap().into();
