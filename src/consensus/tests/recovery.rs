...
async fn recovered_same_term_leader_cannot_serve_a_heartbeat_fence_before_tail_replay() {
    use axum::{
        body::{Body, to_bytes},
        extract::Request,
    ...
    assert_eq!(leader.metrics().current_term, 1);
    // This is the precise pre-fix failure: a fresh heartbeat succeeds while
    // the last previously acknowledged application state is still missing.
    tokio::time::timeout(Duration::from_secs(2), leader.raft.ensure_linearizable())
        .await
        .unwrap()
        .unwrap();
    assert_eq!(leader.store.read_fence().unwrap().applied.index, 0);
