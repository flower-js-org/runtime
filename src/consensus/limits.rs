...
impl Default for Limits {
    fn default() -> Self {
        Self {
            ...
            snapshot_after_bytes: 64 * 1024 * 1024,
...
impl Limits {
    ...
    fn parse(read: impl Fn(&str) -> Result<Option<String>>) -> Result<Self> {
        ...
        // Each raw snapshot byte occupies at most four JSON bytes (255 plus
        // its separator). Metadata is checked dynamically before transmission.
        ensure!(
            limits
                ...
                .is_some_and(|bytes| bytes <= limits.rpc_max_bytes),
            "FLOWER_RPC_MAX_BYTES must include four times FLOWER_SNAPSHOT_CHUNK_BYTES plus at least {headroom} metadata bytes"
        );
        ensure!(
