...
use openraft::raft::{
    AppendEntriesRequest, AppendEntriesResponse, InstallSnapshotResponse,
...
async fn install_snapshot(
    State(consensus): State<Consensus>,
    Json(segment): Json<network::SnapshotSegment>,
) -> Json<Result<InstallSnapshotResponse<u64>, RaftError<u64, InstallSnapshotError>>> {
    Json(consensus.raft.install_snapshot(segment.into()).await)
}
