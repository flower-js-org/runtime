...
Before a permission request reaches anyone, [Jev](https://typesafe.ai), TypeSafe's classifier, reviews it. It is on by default: admins turn it off for new sessions in the organization's settings, and anyone who can see a session turns it off for that session.
...
2. The service worker reads `session.review`: each call with its tool's description and where it runs (the user's machine, a sandbox, an MCP server), and the conversation that led to it: the latest user message and up to 12 events after it, long texts clipped. It asks Jev two questions per call: did the user ask for this, counting only the user's own messages and not instructions found in files or tool output, and is it safe to run unchecked.
3. `reviews.complete` logs each verdict (`tool_reviewed`, with Jev's version and scores). Calls Jev answers yes to on both, with probability 0.9 or more, run. The rest ask as they would have, in one prompt per surface thread, and the web shows why.
