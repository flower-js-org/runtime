...
// the service worker), which sees the call and the conversation that led to it. The calls it
// approves run without asking; the others ask as they would have. A review that fails or
// takes too long asks too, so the reviewer can only ever spare the user a prompt.
...
const unreviewed = (note: string): Verdict => ({ approved: false, reviewer: null, scores: {}, note });
...
function settleReview(tx: Tx, session: Session, verdictOf: (call: Call) => Verdict): void {
  ...
  const steered = session.queued.some((message) => message.steer && message.result === null);
  const asking: Call[] = [];
  for (const call of calls) {
    const { approved, reviewer, scores, note } = verdictOf(call);
    append(tx.ctx, session, { type: "tool_reviewed", call: call.id, approved, reviewer, scores, note });
    if (approved) {
      run(tx, session, call);
    } else if (steered) {
