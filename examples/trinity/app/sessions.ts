...
import { autoApproveAt, newSession, Tx } from "./tx.ts";
...
const settings = {
  ...
  autoApproveAt: v.optional(v.number({ min: 0, max: 1 })),
...
function present(ctx: QueryContext, session: Session) {
  ...
  return { ...session, title: session.title ?? (generated?.status === "ready" ? generated.value : null), autoApproveAt: autoApproveAt(session) };
}
