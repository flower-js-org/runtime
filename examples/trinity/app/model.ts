...
export type Body =
  ...
  | { type: "tool_reviewed"; call: string; approved: boolean; reviewer: string | null; scores: Record<string, number>; note: string | null; threshold?: number }
...
export interface Queued {
  ...
  text: string;
  steer: boolean;
  /** Said in a surface thread without addressing the agent: it joins the turn like a steer, but leaves open prompts alone. */
  aside?: boolean;
  at: number;
...
export interface Session {
  ...
  autoApprove: boolean;
  /** The score, from 0 to 1, that each of the reviewer's answers must reach for a call to run without asking. */
...
export interface OrgSettings {
  ...
  autoApprove: boolean;
  /** The score new sessions' reviewer answers must reach. */
  autoApproveAt: number;
  webTools: boolean;
  graceMs: number;
  contextTokens: number;
...
/** A reviewer's answers on one call; the session's threshold decides whether it runs without asking. */
