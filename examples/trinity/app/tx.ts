...
import { DEFAULT_SETTINGS, settingsOf } from "./orgs.ts";
...
type Configurable = "model" | "system" | "computer" | "private" | "parent" | "source" | "allow" | "autoApprove" | "autoApproveAt" | "webTools" | "graceMs" | "contextTokens";
export type SessionInit = Pick<Session, "id" | "org" | "createdBy"> & Partial<Pick<Session, Configurable>>;

/** The session's autoapproval threshold; sessions created before it was configurable have none. */
export const autoApproveAt = (session: Session): number => session.autoApproveAt ?? DEFAULT_SETTINGS.autoApproveAt;
...
export function newSession(tx: Tx, init: SessionInit): Session {
  ...
  const session: Session = {
    ...
    autoApproveAt: init.autoApproveAt ?? settings.autoApproveAt,
