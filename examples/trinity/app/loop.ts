...
export function receive(tx: Tx, session: Session, message: Queued): void {
  ...
  const turn = session.turn;
  if (turn !== null && message.steer && turn.completion === null && turn.halting === null) {
...
/** Record why a call stopped waiting or running, and who decided, and update its prompt on a surface thread. */
export function markResolved(tx: Tx, session: Session, call: Call, resolution: Resolution, by?: string): void {
  ...
  if (session.source !== null) resolvedOnSurface(tx, session, call.id, resolution, by ?? null);
}
