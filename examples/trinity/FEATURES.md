...
- Create, send, halt, archive and configure sessions. The settings are:
  ...
  - autoapproval by Jev, on unless turned off, and how confident Jev must be (0.75 by default);
  - web tools on or off;
  - halt grace window;
  - context budget.
...
- Sessions read the whole thread by name: a mention brings the replies, other bots' posts, edits and deletions since the session last heard from the thread, and a first mention partway through a thread brings its first message and latest 50.
- Replies while a turn answers in the thread join it at its next step without preempting prompts; otherwise they wait for the next mention, and their author is nudged once.
- Whatever a thread says during one step joins the session as one message.
- 👀 marks the messages being worked on, ✅ moves to the answered one, 🛑 halts the session.
- Approvals are Approve/Deny buttons and questions an Answer form. The cards go once the prompt is settled, wherever that is.
