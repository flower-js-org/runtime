...
// when mentioned (or in a direct message), turns prompts into buttons, and halts on 🛑. It
// reads the rest of a bound thread too, edits and deletions included, by name.
...
export interface SlackMessage {
  ...
  thread: string;
  user: string;
  /** Another bot's name: the bot reads its messages but never answers them. */
  bot: string | null;
  text: string;
  dm: boolean;
  mentioned: boolean;
...
/** An edit or deletion of a message in a thread (or a direct message). */
export interface SlackChange {
  ...
  user: string;
  bot: string | null;
  /** The new text, or what the deleted message said. */
  text: string;
  /** When it changed. */
  at: string;
}
...
const READ_SUBTYPES = new Set([undefined, "file_share", "thread_broadcast", "bot_message"]);

/**
 * A message in a channel, group or direct message, an edit or deletion of one, or null for
 * everything the bot ignores: its own posts, joins and other subtypes, edits that change no
 * text, and changes outside threads, which cannot be bound. Other bots' messages are read but
 * never answered. An edit that newly mentions the bot counts as a new message. Slack escapes
 * &, < and >; the text returned is unescaped, without the bot's mention.
 */
export function parseMessage(event: Record<string, any>, botUser: string): SlackMessage | SlackChange | null {
  if (event.type !== "message" || typeof event.channel !== "string") return null;
  const mention = mentionOf(botUser);
  const dm = event.channel_type === "im";
  const clean = (raw: string) => raw
    .replace(new RegExp(`${mention.source}[ \\t]*`, "g"), "")
    .replaceAll("&lt;", "<").replaceAll("&gt;", ">").replaceAll("&amp;", "&")
    .trim();
  const author = (message: Record<string, any>): { user: string; bot: string | null } | null => {
    if (message.user === botUser || typeof message.ts !== "string") return null;
    if (message.bot_id) return dm ? null : { user: String(message.user ?? message.bot_id), bot: String(message.bot_profile?.name ?? message.username ?? "A bot") };
    return typeof message.user === "string" ? { user: message.user, bot: null } : null;
  };
  const change = (kind: SlackChange["kind"], message: Record<string, any>, raw: string, before: string): SlackChange | null => {
    const who = author(message);
    const threaded = typeof message.thread_ts === "string";
    if (who === null || (!threaded && !dm && !mention.test(raw) && !mention.test(before))) return null;
    return { kind, channel: event.channel, ts: message.ts, thread: threaded ? message.thread_ts : message.ts, ...who, text: clean(raw), at: String(event.event_ts ?? event.ts) };
  };

  let message = event;
  let edited = false;
  if (event.subtype === "message_deleted" || event.message?.subtype === "tombstone") {
    const before = { ts: event.deleted_ts, ...event.previous_message };
    return change("deleted", before, String(before.text ?? ""), "");
  }
  if (event.subtype === "message_changed") {
    message = event.message ?? {};
    const before = String(event.previous_message?.text ?? "");
    const after = String(message.text ?? "");
    // Unfurls and reply counts change a message too, without changing what it says.
    if (!mention.test(after) || mention.test(before) || message.bot_id) return after === before ? null : change("edited", message, after, before);
    edited = true;
  } else if (!READ_SUBTYPES.has(event.subtype)) {
    return null;
  }
  const who = author(message);
  ...
  return {
    ...
    ts: message.ts,
    thread: typeof message.thread_ts === "string" ? message.thread_ts : message.ts,
    ...who,
    text: clean(raw),
    dm,
    mentioned: who.bot === null && mention.test(raw),
    edited,
...
/** Someone's mention of a person (or a workspace-wide user), as Slack writes it. */
const PERSON = /<@([UW][A-Z0-9]+)(?:\|[^>]*)?>/g;
/** A new session reads its thread's first message and at most this many messages in all. */
const HISTORY_LIMIT = 50;
const HISTORY_PAGES = 5;
const attached = (file: { name?: string }) => `(Attached ${file.name ?? "a file"}.)`;
...
export function slackHandlers(clients: { service: Client; worker: Client }, options: Omit<SlackWorkerOptions, "appToken" | "connect">): SlackHandlers {
  ...
  // Names change rarely; a lookup that failed is tried again next time.
  const names = new Map<string, Promise<string>>();
  /** How people in the workspace know someone: their display name, else their full name. */
  function nameOf({ install, token }: Workspace, who: { user: string; bot: string | null }): Promise<string> {
    if (who.bot !== null) return Promise.resolve(who.bot);
    const key = `${install.team}:${who.user}`;
    let name = names.get(key);
    if (name === undefined) {
      name = api<{ user?: { name?: string; real_name?: string; profile?: { display_name?: string; real_name?: string } } }>(token, "users.info", { user: who.user })
        .then(({ user }) => user?.profile?.display_name || user?.profile?.real_name || user?.real_name || user?.name || who.user, () => {
          names.delete(key);
          return who.user;
        });
      names.set(key, name);
  ...
  /** A message's text with the people it mentions by name, as Slack shows them. */
  async function readable(space: Workspace, text: string): Promise<string> {
    const mentioned = [...new Set([...text.matchAll(PERSON)].map((match) => match[1]!))];
    const found = new Map(await Promise.all(mentioned.map(async (user) => [user, await nameOf(space, { user, bot: null })] as const)));
    return text.replace(PERSON, (_, user: string) => `@${found.get(user)}`);
  }

  /** The thread's messages before this one, for a session that has not heard from it: its first message and the latest ones. */
  async function history(space: Workspace, message: SlackMessage): Promise<{ history: Array<{ ts: string; name: string; text: string }>; omitted: number }> {
    const found: Array<{ ts: string; name: string; text: string }> = [];
    try {
      let cursor: string | undefined;
      for (let page = 0; page < HISTORY_PAGES; page++) {
        const result = await api<{ messages?: Array<Record<string, any>>; has_more?: boolean; response_metadata?: { next_cursor?: string } }>(space.token, "conversations.replies", {
          channel: message.channel, ts: message.thread, latest: message.ts, inclusive: false, limit: 200, cursor,
        });
        for (const each of result.messages ?? []) {
          const earlier = parseMessage({ ...each, type: "message", channel: message.channel, channel_type: message.dm ? "im" : "channel" }, space.install.botUser);
          if (earlier === null || "kind" in earlier || earlier.ts === message.ts) continue;
          const text = await readable(space, [earlier.text, ...earlier.files.map(attached)].filter((part) => part !== "").join("\n\n"));
          if (text !== "") found.push({ ts: earlier.ts, name: await nameOf(space, earlier), text: plain(text, 4_000) });
        }
        cursor = result.response_metadata?.next_cursor || undefined;
        if (!result.has_more || cursor === undefined) break;
      }
    } catch (error) {
      log({ slack: "history failed", error: String(error) });
    }
    const kept = found.length <= HISTORY_LIMIT ? found : [found[0]!, ...found.slice(-(HISTORY_LIMIT - 1))];
    return { history: kept, omitted: found.length - kept.length };
  }
  ...
  async function whisper({ token }: Workspace, target: { channel: string; user: string; thread: string | null; dm: boolean }, post: Post): Promise<void> {
    const common = { channel: target.channel, ...(target.thread === null ? {} : { thread_ts: target.thread }), text: post.text, blocks: post.blocks };
  ...
  async function onMessage(team: string, event: Record<string, any>): Promise<void> {
    ...
    const parsed = parseMessage(event, space.install.botUser);
    if (parsed === null) return;
    if ("kind" in parsed) {
      const { channel, thread, ts, user, kind, at } = parsed;
      await service.mutate("slack.change", { team, channel, thread, ts, user, name: await nameOf(space, parsed), kind, text: await readable(space, parsed.text), at },
        { requestId: `slack:${team}:${channel}:${ts}:${kind}:${at}`, retry: true });
      return;
    }
    const message = parsed;
    // Only threads can be bound, so top-level chatter never needs the database.
    if (!message.dm && !message.mentioned && message.thread === message.ts) return;
    const addressed = message.dm || message.mentioned;
    if (addressed) await react(space.token, true, WORKING, message.channel, message.ts);
    // Files come along with messages for the bot; the others' are named.
    const files = addressed ? await attachments(space, message) : { attachments: [], notes: message.files.map(attached) };
    const text = [await readable(space, message.text), ...files.notes].filter((part) => part !== "").join("\n\n");
    if (!addressed && text === "") return;
    const args = {
      team, channel: message.channel, thread: message.thread, ts: message.ts, user: message.user, name: await nameOf(space, message), text,
      dm: message.dm, mentioned: message.mentioned, label: await label(space, message), attachments: files.attachments,
    };
    const requestId = `slack:${team}:${message.channel}:${message.ts}${message.edited ? ":edit" : ""}`;
    let { value } = await service.mutate("slack.receive", args, { requestId, retry: true });
    if (value.outcome === "history") ({ value } = await service.mutate("slack.receive", { ...args, ...await history(space, message) }, { requestId: `${requestId}:history`, retry: true }));
    if (value.outcome === "received") return;
    // A turn working in the thread will read it at its next step.
    if (value.outcome === "joined") {
      await react(space.token, true, WORKING, message.channel, message.ts);
      return;
    }
    if (addressed) await react(space.token, false, WORKING, message.channel, message.ts);
    // Slack shows an ephemeral reply in a thread only once the thread exists; under a lone message it would never be seen.
    const target = { channel: message.channel, user: message.user, thread: message.dm || message.thread !== message.ts ? message.thread : null, dm: message.dm };
    if (value.outcome === "link") await whisper(space, target, connectPost(connectLink(space.install, message.user)));
    if (value.outcome === "nudge" && message.bot === null) {
  ...
  async function render(message: Outbound, posted: (post: Post, call?: string) => Promise<void>, token: string, channel: string, link: string | null, session: string) {
    switch (message.kind) {
      ...
      case "ask":
        ...
        return;
      case "resolved":
        // A settled prompt only clutters the thread; the session's log keeps what was decided.
        await api(token, "chat.delete", { channel, ts: message.card });
        return;
      case "ended":
        return;
    }
