...
const commands: Record<string, (args: string[]) => Promise<void>> = {
  ...
  async slack([action, ...args]) {
    if (action === "setup") {
      ...
      // The workspace connects to the token's organization; check before walking through Slack's pages.
      const claims: { org?: string } = JSON.parse(Buffer.from((env.TRINITY_TOKEN ?? config.token)!.split(".")[1]!, "base64url").toString());
      if (claims.org === undefined) throw new Error("Choose an organization first: trinity org use ID (trinity org list shows yours)");
