...
export interface GatewayOptions {
  ...
  /** The built web client (web/dist). */
  readonly webRoot?: string;
  /** Serves the web client instead of webRoot: Vite's dev server, under `dev`. Calls next when it has nothing to serve. */
  readonly webMiddleware?: (request: IncomingMessage, response: ServerResponse, next: (error?: unknown) => void) => void;
...
async function serveFile(response: ServerResponse, root: string | undefined, path: string, cache = "no-cache"): Promise<void> {
  let file = root === undefined ? null : inside(root, path);
  if (file === null) throw new HttpError(404, "Not found");
  try {
    if ((await stat(file)).isDirectory()) file = join(file, "index.html");
    const data = await readFile(file);
    response.writeHead(200, { "content-type": CONTENT_TYPES[extname(file)] ?? "application/octet-stream", "cache-control": cache });
    response.end(data);
  } catch {
...
export function createGateway(options: GatewayOptions): Server {
  ...
  const web: Handler = (request, response, url) => {
    const middleware = options.webMiddleware;
    if (middleware !== undefined) {
      return new Promise((done, fail) => {
        response.once("close", () => done());
        middleware(request, response, (error) => fail(error ?? new HttpError(404, "Not found")));
      });
    }
    // Vite names built assets by their content, so they never change.
    const cache = url.pathname.startsWith("/assets/") ? "public, max-age=31536000, immutable" : "no-cache";
    return serveFile(response, options.webRoot, url.pathname === "/" ? "index.html" : url.pathname, cache);
  };
