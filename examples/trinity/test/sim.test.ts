...
import { DEFAULT_SETTINGS } from "../app/orgs.ts";
...
test("simulated reviews approve most calls, the same way every time", async () => {
  ...
  const approved = Object.values(verdicts).filter((verdict) => verdict.scores.safe! >= DEFAULT_SETTINGS.autoApproveAt).length;
