...
test("Jev scores each call in the conversation that led to it", async () => {
  const jev = fakeJev((state) => state.call.id === "c1" ? answer({ safe: 0.9512 }) : answer({ safe: 0.03 }));
  const outcome = await reviewCalls(request, { apiKey: "ts_key", fetch: jev.fetchImpl }, signal);
  assert.deepEqual(outcome.verdicts, {
    c1: { reviewer: "jev-2026-09-20", scores: { safe: 0.951 }, note: null },
    c2: { reviewer: "jev-2026-09-20", scores: { safe: 0.03 }, note: null },
  });
  ...
  assert.deepEqual(Object.keys(first!.body.questions), ["safe"]);
...
test("without a key, or when Jev fails, calls are left to the user; overloads are retried", async () => {
  ...
  assert.deepEqual((await reviewCalls(one, undefined, signal)).verdicts.c1, { reviewer: null, scores: {}, note: "No reviewer is configured." });
  ...
  const overloaded = fakeJev((_, attempt) => attempt === 1 ? json({ error: { message: "overloaded" } }, 529) : answer({ safe: 0.99 }));
  assert.deepEqual((await reviewCalls(one, { apiKey: "k", fetch: overloaded.fetchImpl }, signal)).verdicts.c1!.scores, { safe: 0.99 });
  assert.equal(overloaded.requests.length, 2);

  const garbled = fakeJev(() => answer({ unsafe: 0.99 }));
  const verdict = (await reviewCalls(one, { apiKey: "k", fetch: garbled.fetchImpl }, signal)).verdicts.c1!;
  assert.deepEqual([verdict.reviewer, verdict.note], [null, "Jev did not answer safe"]);
});
