...
where C::SnapshotData: tokio::io::AsyncRead + tokio::io::AsyncWrite + tokio::io::AsyncSeek + Unpin
{
        ...
        Net: RaftNetwork<C> + ?Sized,
    {
        ...
        let mut offset = 0;
        // Flower: the end is found by reading to it rather than by seeking,
        // so that an image can stream while it encodes. A segment that ends
        // exactly at the end is followed by an empty one marked done.
