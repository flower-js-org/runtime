    ...
    SM: RaftStateMachine<C>,
{
    ...
    /// Flower: submit a leader's own append without waiting for its flush.
    ///
    /// Raft permits a leader to write its log in parallel with replication
    /// (Ongaro's thesis, section 10.2.1). Upstream 0.9 awaits the flush here,
    /// so `Replicate` commands queued behind `AppendInputEntries` wait for the
    /// leader's fsync and the follower fsync starts only afterwards. Storage
    /// still makes entries readable when `append()` returns and still orders
    /// later storage I/O behind this append. The leader's own matching log
    /// advances only on `Notify::LocalLogFlushed`, after durability, so a
    /// commit still requires a durable quorum.
    #[tracing::instrument(level = "debug", skip_all)]
    async fn submit_leader_append<I>(
        &mut self,
        entries: I,
        vote: Vote<C::NodeId>,
        last_log_id: LogId<C::NodeId>,
    ...
    {
        let (tx, rx) = C::AsyncRuntime::oneshot();
        let callback = LogFlushed::new_leader(LogIOId::new(vote.clone(), Some(last_log_id.clone())), tx);
        self.log_store.append(entries, callback).await?;
        ...
        let _ = C::AsyncRuntime::spawn(async move {
            let result = match rx.await {
                Ok(Ok(_)) => Ok(()),
                Ok(Err(e)) => Err(StorageIOError::write_logs(AnyError::error(e)).into()),
                Err(e) => Err(StorageIOError::write_logs(AnyError::error(e)).into()),
            };
            let _ = tx_notify.send(Notify::LocalLogFlushed {
                vote,
                log_id: last_log_id,
                result,
            });
    ...
    pub(crate) async fn handle_api_msg(&mut self, msg: RaftMsg<C>) {
        ...
        match msg {
            ...
            // Flower: see RaftMsg::SnapshotChunk.
            RaftMsg::SnapshotChunk { vote, tx } => {
                self.engine.handle_snapshot_chunk(vote, tx);
            }
    ...
    pub(crate) fn handle_notify(&mut self, notify: Notify<C>) -> Result<(), Fatal<C::NodeId>> {
        ...
        match notify {
            ...
            Notify::LocalLogFlushed { vote, log_id, result } => {
                result?;
                // A flush submitted under an older leadership proves nothing
                // for the current one; later appends carry their own proof.
                if self.engine.leader.as_ref().is_some_and(|leader| leader.vote == vote) {
    ...
    async fn run_command(&mut self, cmd: Command<C>) -> Result<Option<Command<C>>, StorageError<C::NodeId>> {
        ...
        match cmd {
            ...
            Command::AppendInputEntries { vote, entries } => {
                ...
                // Flower: the current leader's own entries replicate while its
                // flush runs; see submit_leader_append(). Followers still block
                // so their AppendEntries responses continue to imply durability.
                if self.engine.leader.as_ref().is_some_and(|leader| leader.vote == vote) {
                    self.submit_leader_append(entries, vote, last_log_id).await?;
                    return Ok(None);
                }
