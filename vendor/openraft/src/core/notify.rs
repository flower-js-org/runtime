...
where C: RaftTypeConfig
{
    ...
    /// Flower: a leader's own log append, submitted without blocking
    /// `RaftCore`, is durable (or failed). Only now does the leader count
    /// itself toward the commit quorum for entries up to `log_id`.
    LocalLogFlushed {
        /// The leader vote under which the entries were appended.
        vote: Vote<C::NodeId>,
        log_id: LogId<C::NodeId>,
        result: Result<(), StorageError<C::NodeId>>,
    },
}
...
where C: RaftTypeConfig
{
    fn summary(&self) -> String {
        match self {
            ...
            Self::LocalLogFlushed { vote, log_id, result } => {
                format!(
                    "LocalLogFlushed: vote: {}, log_id: {}, ok: {}",
                    vote,
                    log_id,
                    result.is_ok()
                )
