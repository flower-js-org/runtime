...
where C: RaftTypeConfig
{
    ...
    /// Flower: a chunk of a snapshot from the leader of `vote`. It is contact from
    /// that leader, as an append is, so that a transfer longer than the
    /// election timeout does not start an election that restarts it.
    SnapshotChunk {
        vote: Vote<C::NodeId>,
        tx: ResultSender<C, SnapshotResponse<C::NodeId>>,
    },
...
where C: RaftTypeConfig
{
    fn summary(&self) -> String {
        match self {
            ...
            RaftMsg::SnapshotChunk { vote, .. } => {
                format!("SnapshotChunk: vote: {}", vote)
            }
