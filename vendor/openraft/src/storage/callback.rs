...
where C: RaftTypeConfig
{
    ...
    // Flower: the current leader's own append, which does not block replication.
    leader: bool,
}
...
where C: RaftTypeConfig
{
    ...
    /// Flower: a callback for the current leader's own append. Replication does
    /// not wait for it; the leader counts itself toward the commit quorum only
    /// once it completes.
    ...
    /// Flower: whether this is the current leader's own append, which storage
    /// may make durable later than a follower append.
    pub fn is_leader_append(&self) -> bool {
        self.leader
    }
