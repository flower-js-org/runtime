...
where C: RaftTypeConfig
{
        ...
        C::SnapshotData: tokio::io::AsyncRead + tokio::io::AsyncWrite + tokio::io::AsyncSeek + Unpin,
    {
        ...
        // Flower: accept the leader through the core, as an append does, so that its
        // lease covers the transfer. Once accepted, the local vote is the
        // leader's; any other vote rejected the chunk.
        let (tx, rx) = C::AsyncRuntime::oneshot();
        let accepted = self
            .inner
            .call_core(RaftMsg::SnapshotChunk { vote: req_vote.clone(), tx }, rx)
            .await
            .map_err(|e| RaftError::Fatal(e.into_fatal().unwrap()))?;
        if accepted.vote != req_vote {
            return Ok(InstallSnapshotResponse { vote: accepted.vote });
        }
        ...
        if let Some(snapshot) = finished_snapshot {
            // Flower: the leader awaits this response and sends nothing else, and
            // installing can outlast the election timeout too: keep
            // accepting the leader's vote until the install is done.
            let install = std::pin::pin!(self.install_full_snapshot(req_vote.clone(), snapshot));
            // The core applies each vote when it reads the message, but its
            // replies queue behind the install's: send without awaiting them.
            let keepalive = std::pin::pin!(async {
                let interval = Duration::from_millis(self.inner.config.heartbeat_interval);
                loop {
                    C::AsyncRuntime::sleep(interval).await;
                    let (tx, _rx) = C::AsyncRuntime::oneshot();
                    let vote = req_vote.clone();
                    if self.inner.send_msg(RaftMsg::SnapshotChunk { vote, tx }).await.is_err() {
                        std::future::pending::<()>().await;
                    }
                }
            });
            let resp = match futures::future::select(install, keepalive).await {
                futures::future::Either::Left((resp, _)) => resp?,
                futures::future::Either::Right(((), _)) => unreachable!("keepalive never ends"),
            };
            return Ok(resp.into());
        }
        Ok(resp)
