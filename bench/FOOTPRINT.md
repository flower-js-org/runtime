...
Three nodes with 64 MiB caches, 100,000 orders, one follower stopped at 50,000 (`--catch-up`): the leader stayed at 417 MiB and the other follower at 272 MiB. The restarted follower installed a snapshot of the whole state, held in a 641 MiB database, and caught up in 16.4 s with a peak resident set of 285 MiB. In the same run at 40,000 orders, main's follower had not caught up after several minutes: each election restarted its transfer.
...
- **Scan windows.** Each cell that scans a range keeps its windows in memory.
