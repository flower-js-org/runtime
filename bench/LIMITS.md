...
| `FLOWER_SNAPSHOT_CHUNK_BYTES` | 1048576 | Raw snapshot bytes per transport segment, sent as base64. `FLOWER_RPC_MAX_BYTES` must hold four thirds of it plus metadata. |
| `FLOWER_VALUE_CACHE_BYTES` | 268435456 (256 MiB) | Parsed stored values shared by every reader in the process, charged at six times their JSON plus 64 bytes. It evicts in insertion order; a value still in use stays alive, uncached. |
...
| `FLOWER_SNAPSHOT_AFTER_BYTES` | 16777216 (16 MiB) | Soft trigger for encoded durable log bytes applied after the last published snapshot. Zero disables this trigger. A published snapshot pins the redb pages it reads, so the database file grows with the pages freed until the next one, many times these log bytes. |
