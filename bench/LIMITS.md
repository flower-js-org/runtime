...
A transaction's graph charge is the accounted size of each cell or root entry it adds or replaces: for a cell, its dependency list and reverse edges plus room for its proof and traversal entries; for a root, its lookup and append-frontier entries. Entries it keeps unchanged from its snapshot cost nothing, and removing an entry releases only what the same transaction charged for it. Inserting rows into a large database therefore costs what the new rows add, whatever the size of the graph. Validating a restored graph, or the whole graph after a removal or a changed derived edge, builds a reachability proof proportional to the graph, a few hundred bytes per cell. That traversal is bounded by the evaluation deadline and by process memory, not charged to the transaction that triggers it. A restored graph's proof stays with the snapshot for later transactions.
