...
function histogramTable(rows, caption) {
  return table(["Measurement", "Samples", "Min", "p50", "p95", "p99", "Max", "Invalid / overflow"], rows.map(([name, histogram]) => {
    const values = object(histogram);
    return [escape(name), count(values.samples), ms(values.min), ms(values.p50), ms(values.p95), ms(values.p99), ms(values.max), `${count(values.invalidSamples)} / ${count(values.overflowSamples)}`];
  }), { caption });
}
...
function phaseDetails(name, raw) {
  ...
  const httpRows = methods.map(([method, rawValue]) => {
    ...
    const latency = object(value.latencyMs);
    return [escape(method), count(value.attempts), count(value.successes), count(value.failures), count(value.retries), count(value.duplicates), number(value.throughputPerSecond), ms(latency.p50), ms(latency.p95), ms(latency.p99), ms(latency.max)];
  });
  const logicalRows = logicalMethods.map(([method, rawValue]) => {
    const value = object(rawValue);
    const latency = object(value.latencyMs);
    return [escape(method), count(value.count), count(value.completed), count(value.failed), count(value.duplicates), number(value.throughputPerSecond), ms(latency.p50), ms(latency.p95), ms(latency.p99), ms(latency.max)];
  });
  ...
  ])}</div><p class="small"><strong>HTTP error categories:</strong> ${errorList(phase.errors)}</p>${table(["HTTP method", "Attempts", "Succeeded", "Failed", "Retries", "Duplicates", "Success / s", "p50", "p95", "p99", "Max"], httpRows, { caption: `${label(name)} HTTP attempts. All latency columns are milliseconds.` })}<details class="nested-detail"><summary>Logical calls, including explicit replays</summary>${table(["Logical method", "Calls", "Completed", "Failed", "Duplicates", "Complete / s", "p50", "p95", "p99", "Max"], logicalRows, { caption: "One logical call includes discovery, retries, and backoff. Explicit replay checks are separate .replay calls." })}</details></div></details>`;
}
...
export function renderReport(rawReport, { baseline: rawBaseline } = {}) {
  ...
  const latencyGroups = phases.map(([name, raw]) => {
    const latency = object(object(raw).operations?.latencyMs);
    return { name: label(name), values: [latency.p50, latency.p95, latency.p99] };
  });
  const lifecycleGroups = [["Oven lateness", lifecycle.timerLatenessMs], ["Order to door", lifecycle.orderToDeliveryMs]].map(([name, raw]) => {
    const value = object(raw);
    return { name, detail: `${count(value.samples)} samples`, values: [value.p50, value.p95, value.p99] };
  });
...
<main id="results"><div class="metrics">${metric("Customer goodput", `${number(application.goodputRps)} <small>/ s</small>`, `${percent(application.reads.fraction)} reads · ${percent(application.mutations.fraction)} mutations among successful customer calls.`, "feature")}${metric("All-call tail latency", `${number(logical.latencyMs?.p99)} <small>ms</small>`, "All load-phase logical calls, including worker traffic, replays, discovery, and retries.")}${metric("Orders delivered", `${count(audit.delivered)} <small>/ ${count(audit.orders)}</small>`, `${count(audit.pizzas)} pizzas · ${count(audit.revenue)} copper in delivery revenue.`)}${metric("Quorum recovery", chaos.length ? `${number(recovery)} <small>ms</small>` : "Not exercised", chaos.length ? "First recorded crash to a serving quorum; restart catch-up is tracked separately." : "Leader failure injection was not recorded in this run.")}</div>
...
<section class="panel"><div class="section-heading"><div><p class="eyebrow">From the oven to the doorstep</p><h2>Business completion latency</h2></div><span class="tag">Load and drain combined</span></div><div class="charts-grid"><div>${groupedChart("lifecycle", "Oven lateness and order-to-delivery percentiles", "Oven lateness is callback execution time minus its due time. Order-to-delivery includes baking, queueing, abandoned leases, and delivery.", lifecycleGroups, ["p50", "p95", "p99"], "ms")}</div><div>${definitionList([["Configured oven delay", ms(options.bakeMs)], ["Configured lease duration", ms(options.leaseMs)], ["Delivered orders / s, including drain", number(lifecycle.deliveredOrdersPerSecondIncludingDrain)], ["Retained order cap", count(options.maxOrders)]])}<p class="note">Oven lateness measures how far execution runs past the requested deadline. It does not include the configured baking delay. Order-to-door latency includes the entire lifecycle. These are recorded quantiles; raw histogram buckets are not present in this report.</p></div></div>${histogramTable([["Oven lateness", lifecycle.timerLatenessMs], ["Order to delivery", lifecycle.orderToDeliveryMs]], "Approximate nearest-rank quantiles. Min and max are exact observed values.")}</section>
